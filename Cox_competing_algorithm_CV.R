  ...
  repeat {
    ...
    for (r in 1:n_draw) {
      ...
      if (is.null(model_res)) {
        ...
        iauc_vec[r] <- compute_iAUC(transformed_data, result = model_res, time_points = quantile(transformed_data$time,seq(0.1,0.9,0.05)))
      }
...
high_select_cv <- function(time, status, X, nfold = 5, n_group = 3, size_group = 3, init_iauc = 0.5, in_iauc = 0.05, tar_P = 15, n_draw = 100){
  ...
  if(tar_P < n_group*size_group){
    
    message(
      "Target size (",
      tar_P,
      ") is smaller than maximum sampled size (",
      n_group*size_group,
      "). Stop screening."
    )
  ...
  repeat{message( "Iteration ",m, " | Candidate genes ",  length(candidate_pool),   " | threshold ", round(zeta,3)
    )
    
    retained_covariates <- list()
    retained_labels <- list()
    ##############################################
    # Random screening
    ##############################################
    ...
    for(r in 1:n_draw){
      ...
      group_sizes <- sample(1:size_group,  n_group,  replace=TRUE)
      
      n_selected <- sum(group_sizes)
      
      selected_idx <- sample(candidate_pool, n_selected, replace = FALSE)
      ...
      for(k in 1:nfold){
        ...
        train_comp <- data_transform(time[train_idx],status[train_idx],X[train_idx,selected_idx],label_draw)
        
        val_comp <- data_transform(time[val_idx], status[val_idx],
          X[val_idx,selected_idx], label_draw)
        
        fit <- tryCatch({suppressMessages(Cox_com_fit( train_comp, maxit=1000, tolerance=1e-3))},error=function(e){
          NULL
        })
        ...
        if(is.null(fit)){
          ...
          eval_time <- quantile(time[val_idx],seq(0.1,0.9,0.05))
        
          fold_auc[k] <- tryCatch({compute_iAUC(val_comp, fit, eval_time)
      ...
      ##############################################
      # CV performance of this random subset
      ##############################################
      ...
      if(mean_auc >= zeta){
        ...
        retained_covariates <- c(retained_covariates, list(selected_idx))
        retained_labels <- c(retained_labels,list(label_draw))
      }
  ...
  return(
    list(selected_indices=candidate_pool, X_selected=X[,candidate_pool,drop=FALSE],
      n_selected=length(candidate_pool), threshold=zeta, retained_labels = retained_labels, retained_covariates=retained_covariates
    )
  )
...
lasso_competing_select <- function(time, status, X, n_group = 3,
                                   penalty = 0.1, n_draw = 100){
  
  n_covariates <- ncol(X)
  cov_names <- colnames(X)
  
  selected_list <- vector("list", n_draw)
  
  message("Starting LASSO competing Cox selection")
  message("CF number: ", n_group)
  message("Repeats: ", n_draw)
  
  
  for(r in 1:n_draw){
    
    message("Iteration ", r, "/", n_draw)
    
    #################################
    # Random allocation of CFs
    #################################
  
    label_draw <- as.list(
      sample(rep(1:n_group, length.out = n_covariates))
    )
    
    transformed_data <- data_transform(time, status,
      X = as.matrix(X), label = label_draw)
    ...
    if(is.null(model_res)){
      selected_list[[r]] <- character(0)
      next
    }
    

    #################################
    # Extract non-zero coefficients and CF labels
    #################################
    ...
    for(l in 1:n_group){
      ...
      beta_l <- model_res$param_new[[l]]$beta
      
      if(length(beta_l)>0){
        
        nz <- which(abs(beta_l) > 1e-8)
        
        if(length(nz)>0){
          group_vars <- colnames(transformed_data$X[[l]])
          
          selected <- rbind(
            selected,
            data.frame(
              variable = group_vars[nz],
              CF = l
            )
          )
  ...
  all_selected <- do.call(
    rbind,
    selected_list
  )
  
  freq <- table(
    all_selected$variable
  )
