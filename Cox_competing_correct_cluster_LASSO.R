...
update_l <- function(time, status, X_l, eta_l, b_hat, var_b, penalty = 0){
    
    pseudo_data <- make_pseudo(time, status, X_l, eta_l, b_hat, var_b)
    
    covariate_names <- names(pseudo_data)[!(names(pseudo_data) %in% c("time", "status", "eta", "b_hat"))]
    ...
    if(penalty == 0){
      ...
      beta_l <- fit_beta$coefficients
      
    } else {
      # LASSO weighted Cox
      
      fit_beta <- glmnet::glmnet(
        x = as.matrix(pseudo_data[, covariate_names, drop = FALSE]),
        y = survival::Surv(pseudo_data$time, pseudo_data$status),
        family = "cox",
        alpha = 1, #LASSO
        lambda = penalty,
        weights = pseudo_data$eta,
        offset = if(is.null(b_hat)) NULL else pseudo_data$b_hat
      )
      
      beta_l <- as.numeric(coef(fit_beta))
      names(beta_l) <- covariate_names
    }
    ...
    if(penalty > 0){
      # construct Cox object only for baseline hazard calculation
      if(is.null(b_hat)){
        
        fit_base <- survival::coxph(
          survival::Surv(pseudo_data$time, pseudo_data$status) ~ as.matrix(pseudo_data[, covariate_names, drop = FALSE]),
          weights = pseudo_data$eta,
          init = beta_l,
          iter.max = 0,
          ties = "breslow"
      ...
      bh <- survival::basehaz(fit_base, centered = FALSE)
      
    } else {
      bh <- survival::basehaz(fit_beta, centered = FALSE)
    }
    ...
    return(list(bh_times = bh$time, lambda_l = lambda_l, cum_lambda_l = cum_lambda_l,beta_l = beta_l))
}
